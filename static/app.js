...
function deleteChatMessage(msg) {
    ...
    fetch(`${API_BASE}/rooms/${state.activeServerId}/messages/delete`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ channelId: msg.channelId, msgId: msg.id })
    }).catch(() => {});
    renderMessages();
}
...
saveProfessionalServerSettings = window.saveProfessionalServerSettings = function () {
    ...
    if (icon !== undefined) {
        fetch(`${API_BASE}/rooms/${server.id}/icon`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url: icon || null })
        }).catch(() => {});
    }
    ...
    meshBroadcastReliable({ type: "server_update", payload: { id: server.id, name: server.name, icon_url: server.icon_url, roles: server.roles, peer_roles: server.peer_roles, channel_backgrounds: server.channel_backgrounds || {} } });
    renderServerRail();
...
function leaveServer(serverId) {
    ...
    if (idx === -1) return;
    // Clean up voice before disconnecting
    if (state.voiceChannelId) {
        meshBroadcastReliable({ type: "voice_leave", channelId: state.voiceChannelId || "" });
        sendServerEvent({ type: "voice_leave", channelId: state.voiceChannelId });
    }
    state.voiceChannelId = null;
...
function createSidebarItem(name, color, image, onclick, peerId) {
    ...
    if (peerId) {
        item.oncontextmenu = (e) => showDMSidebarContextMenu(e, peerId, name);
    }
    return item;
}
...
function closeDM(peerId) {
    if (!state.recentDMs) state.recentDMs = [];
    state.recentDMs = state.recentDMs.filter(d => d.peerId !== peerId);
    localStorage.setItem("scord_recent_dms", JSON.stringify(state.recentDMs));
    if (state.activeDM === peerId) hideDMMainView(true);
    if (!state.activeServerId) renderHomeSidebar();
    toast("DM kapatildi.", "info");
}

function deleteDMChat(peerId) {
    if (!state.dms) state.dms = {};
    delete state.dms[peerId];
    if (state.activeDM === peerId) renderDMMessages(peerId);
    toast("DM konusmasi temizlendi.", "success");
}

function showDMSidebarContextMenu(e, peerId, name) {
    e.preventDefault();
    e.stopPropagation();
    let menu = document.getElementById("dm-sidebar-ctx");
    if (menu) menu.remove();
    menu = document.createElement("div");
    menu.id = "dm-sidebar-ctx";
    menu.className = "context-menu";
    menu.style.cssText = "position:fixed;z-index:9999;min-width:180px;";
    menu.style.left = `${e.clientX}px`;
    menu.style.top = `${e.clientY}px`;
    menu.innerHTML = `
        <div class="ctx-header" style="padding:8px 12px;font-weight:600;border-bottom:1px solid var(--border);margin-bottom:4px">${escapeHtml(name)}</div>
        <div class="ctx-item" data-action="close-dm">DM'yi Kapat</div>
        <div class="ctx-item ctx-danger" data-action="delete-dm">Konusmayi Sil</div>
    `;
    menu.querySelector('[data-action="close-dm"]').onclick = () => { menu.remove(); closeDM(peerId); };
    menu.querySelector('[data-action="delete-dm"]').onclick = () => { menu.remove(); if (confirm("Bu konusmadaki tum mesajlar silinsin mi?")) deleteDMChat(peerId); };
    document.body.appendChild(menu);
...
function renderDMMessages(peerId) {
    ...
    messages.forEach(msg => {
        ...
        el.oncontextmenu = (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (msg.authorId !== state.peerId) return;
            let menu = document.getElementById("dm-msg-ctx");
            if (menu) menu.remove();
            menu = document.createElement("div");
            menu.id = "dm-msg-ctx";
            menu.className = "context-menu";
            ...
            menu.style.left = `${e.clientX}px`;
            menu.style.top = `${e.clientY}px`;
            menu.innerHTML = `<div class="ctx-item ctx-danger">Mesaji Sil</div>`;
            menu.querySelector(".ctx-item").onclick = () => {
                menu.remove();
                const msgs = state.dms[peerId];
                if (msgs) {
...
function updateServerIcon(serverId, url) {
    fetch(`${API_BASE}/rooms/${serverId}/icon`, {
        ...
        if (data.success) {
            ...
            if (server) {
                server.icon_url = url;
                meshBroadcastReliable({ type: "server_update", payload: { id: server.id, icon_url: url, name: server.name, roles: server.roles, peer_roles: server.peer_roles, channel_backgrounds: server.channel_backgrounds || {} } });
                sendServerEvent({ type: "server_update", payload: { id: server.id, icon_url: url } });
            }
            renderServerRail();
        }
    });
...
saveProfessionalServerSettings = window.saveProfessionalServerSettings = function () {
    ...
    if (server) {
        const iconInput = document.getElementById("settings-sv-icon");
        if (iconInput) {
