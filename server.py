...
def update_icon(room_id: str, body: dict):
    ...
    save_db()
    return {"success": True, "icon_url": room.icon_url}
...
def delete_channel(room_id: str, channel_id: str):
    if room_id not in rooms: return {"error": "Not found"}
    room = rooms[room_id]
    room.channels = [ch for ch in room.channels if ch["id"] != channel_id]
    room.messages.pop(channel_id, None)
    getattr(room, "channel_permissions", {}).pop(channel_id, None)
    getattr(room, "channel_backgrounds", {}).pop(channel_id, None)
    save_db()
    return {"success": True}

@app.patch("/api/rooms/{room_id}/channels/{channel_id}")
def rename_channel(room_id: str, channel_id: str, body: dict):
    if room_id not in rooms: return {"error": "Not found"}
    room = rooms[room_id]
...
@app.post("/api/rooms/{room_id}/messages/delete")
def delete_message(room_id: str, body: dict):
    if room_id not in rooms: return {"error": "Not found"}
    room = rooms[room_id]
    channel_id = body.get("channelId")
    msg_id = body.get("msgId")
    if not channel_id or not msg_id: return {"error": "Missing channelId or msgId"}
    if channel_id in room.messages:
        room.messages[channel_id] = [m for m in room.messages[channel_id] if m.get("id") != msg_id]
    room.pinned_messages = [m for m in room.pinned_messages if m.get("id") != msg_id]
    save_db()
    return {"success": True}
